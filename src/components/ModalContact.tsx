...
export const ModalContact = ({ onClose, open }: Props) => {
  return (
    <div onClick={onClose} className={`fixed inset-0 flex justify-center items-center transition-colors ${open ? "visible bg-black/20" : "invisible"}`}>
      ...
      className={`w-screen h-screen md:h-[100%] md:w-[700px] bg-white rounded-xl shadow py-[200px] md:py-[145px] px-[45px] transition-all ${open ? "scale-100 apacity-100" : "scape-125 opacity-0"}`}>
        <FormContact />
